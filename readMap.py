...
class MapHandler(xml.sax.ContentHandler):
	...
	def startElement(self, tag, attributes):
		...
		if (self.Current_Tag == 'way') and (tag == 'nd') and (attributes.has_key('ref')):
			self.Way_Nodes = self.Way_Nodes + attributes['ref'] + ':'
		...
		if (self.Current_Tag == 'relation') and (tag == 'member') and (attributes.has_key('type')):
			if attributes['type'] == 'way':
				self.Relation_Ways = self.Relation_Ways + attributes['ref'] + ':'
			if attributes['type'] == 'node':
				self.Relation_Nodes = self.Relation_Nodes + attributes['ref'] + ':'
	...
	def endElement(self,tag):
		...
		if tag == 'way':
			self.cur.execute("insert into Way values\
				('"+self.Way_ID+"','"+self.Way_Name+"','"+self.Way_Nodes+"')")
			#self.conn.commit()
		...
		if tag == 'relation':
			self.cur.execute("insert into Relation values\
				('"+self.Relation_ID+"','"+self.Relation_Name+"','"+self.Relation_Nodes+"','"+self.Relation_Ways+"')")
			#self.conn.commit()
